...
pool_board* create_pool(int num_threads, board_t* b){
    ...
    init_wait_group_sema(&pb->wg_done);
...
void *thread_work(void *t_arg) {
    ...
    while(1) {
        ...
        wait_group_done_sema(&pb->wg_done);
    }
    pthread_exit(NULL);
}
...
void iterate_concurrently(pool_board* pb){
    ...
    wait_group_add_sema(&pb->wg_done,pb->b->sideLenght);
    ...
    wait_group_wait_sema(&pb->wg_done);
...
void destroy_pool(pool_board* pb){
    ...
    destroy_wait_group_sema(&pb->wg_done);
}
